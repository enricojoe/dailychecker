...
- [~] Create bot via BotFather (manual, documented) — **deferred (needs real token)**; steps in [m5-telegram.md](m5-telegram.md)
- [x] `POST /api/telegram/link` issues one-time token + `t.me/<bot>?start=<token>` URL
- [x] Bot update handler (long-poll for dev) captures `/start <token>` → saves `chat_id`
- [x] Message-send helper with basic rate-limit safety
- [x] Bot includes a link back to the web app
- **DoD:** A real user can connect their account; backend stores `chat_id`; a test send delivers a DM. _(Code-complete & proven via mocked tests; live DM delivery with a real BotFather token is the one deferred step — see manual steps in the M5 plan.)_
- **Result:** _2026-06-20_ — Completed & verified against a MOCKED Telegram API (see [m5-telegram.md](m5-telegram.md)); live delivery deferred. `telegram.Client` interface + real `httpClient` (injected `*http.Client` + overridable base URL → tests hit an `httptest.Server`, no real network; M6 scheduler will reuse the interface). Rate-limit: 50ms min-interval throttle + HTTP 429 → `ErrRateLimit` sentinel preserving `retry_after` (caller owns retry). `telegram.Service`: `IssueLink` (32-byte crypto-random token via `crypto/rand`, stored through `users.Update`, returns `t.me/<username>?start=<token>`) and `HandleStart` (looks up by token, sets chat_id + linked_at, clears token single-use, sends confirmation DM with `AppPublicURL`; unknown/empty token → no-op so the poll loop never errors). `telegram.Poller` long-polls `getUpdates` with offset tracking + `/start <token>` parsing, cleanly stopped via context. New users repo method `GetByLinkToken`. `POST /api/telegram/link` (protected) registered only when a bot token is configured; `main.go` constructs the telegram stack + starts the poller goroutine only if `TELEGRAM_BOT_TOKEN != ""`, cancels it on graceful shutdown, and logs "telegram: disabled" otherwise. Tokens never logged. 17 M5 tests (telegram + httpapi), all mocked/deterministic; `go build`/`go vet` clean; 2 parallel `go test ./...` runs green. **Orchestrator also verified the server boots with an empty `TELEGRAM_BOT_TOKEN`** (healthz 200, "telegram: disabled" logged, link route correctly absent from route table).

### Milestone 6 — Scheduler (per-activity reminders + nightly digest)
- [x] `scheduler` package with `robfig/cron`, isolated behind an interface
- [x] Minute-tick: find activities due now & not done → send per-activity reminder once (`per_activity_notified_at`)
- [x] Nightly digest job at 22:00 (env `DIGEST_HOUR`): per user, list all not-done occurrences → single message (`digest_notified_at`)
- [x] Only notify users with `telegram_chat_id`; all times in Jakarta
- [x] Tests with injected clock; dedup so no double-send
...
- **Result:** _2026-06-20_ — Completed & verified (see [m6-scheduler.md](m6-scheduler.md)). `internal/scheduler.Scheduler` with an injected clock (`func() time.Time`) + Jakarta `*time.Location`; two directly-callable core methods `RunReminderTick(ctx, now)` and `RunDigest(ctx, now)` so tests drive exact times with no cron/sleep. `Start(ctx)` wires `robfig/cron/v3` (Jakarta loc): minute-tick → reminders, daily at `DigestHour:00` → digest; `Stop()`/context-cancel halts cleanly. Reminder selection: top-level active activities with `time_of_day <= now`, occurrence not done, `per_activity_notified_at IS NULL`, owner has chat id → send once, set the flag (`<=` makes a reminder missed during downtime still fire next tick, still exactly once via the flag). Digest: per chat-id user, all not-done occurrences with `digest_notified_at IS NULL` → one message incl. `AppPublicURL`, mark each. Reuses `telegram.Client` (no direct HTTP); new JOIN-based occurrences repo methods `ListDueReminders`/`MarkPerActivityNotified`/`ListDigestItems`/`MarkDigestNotified` (no N+1). Wired in `main.go` with the real clock, started only when `TELEGRAM_BOT_TOKEN` set, stopped on shutdown. 9 scheduler tests (injected clock + mock client + real DB): reminder fires-once/respects-time/skips done+no-chatid+sub-activities; digest one-per-user/idempotent/skips done+no-chatid. Dep `github.com/robfig/cron/v3` added by orchestrator (subagents can't `go get` — no network; see [workflow lesson](workflow.md)). `go build`/`go vet` clean; 2 parallel `go test ./...` runs green; **orchestrator confirmed server still boots tokenless with the scheduler not started**.

### Milestone 7 — Frontend: Auth & Shell
- [x] Login/register pages; token storage; auto-refresh on 401; logout
- [x] App shell, routing, protected routes, API client + TanStack Query setup
- **DoD:** User can register, log in, stay logged in across refresh, and log out.
- **Result:** _2026-06-20_ — Completed & verified (see [m7-frontend-auth-shell.md](m7-frontend-auth-shell.md)). `react-router-dom@7` added by orchestrator. `tokenStore.ts` persists access+refresh in localStorage (survives hard refresh). `apiClient.ts` extended: Bearer injection + **single-flight** 401→refresh→retry (shared `refreshPromise` so parallel 401s don't invalidate each other since refresh rotates), clears tokens + redirects to /login on refresh failure, handles 204. `AuthContext`/`useAuth` hydrate user via a `GET /api/me` query enabled when a token exists; login/register mutations; logout calls API + clears + resets query cache. Routes: public `/login` `/register` (redirect to `/` if authed), protected `/` (AppShell + TodayPlaceholder) via `ProtectedRoute`, `*` catch-all. Accessible controlled forms surface backend `{error}`. **Orchestrator fixes after subagent (which couldn't run the toolchain):** removed a TS parameter-property in `ApiError` (broke `erasableSyntaxOnly` build), added react-refresh eslint-disable on `useAuth` + pre-existing `button.tsx` exports, corrected `UserDto.id` `number`→`string` (backend ids are UUIDs). `npx tsc --noEmit`, `npm run build`, `npm run lint` all clean. **Live integration verified by orchestrator** against the running backend: register→login→/me(+401 unauth)→refresh(rotation: old refresh→401)→logout(204) all match the frontend's request shapes. (Full browser click-through of localStorage persistence/route guards validated by code review; not automated.)

### Milestone 8 — Frontend: Activities, Today, History, Telegram
- [x] Activity CRUD UI incl. schedule editor + sub-activities
- [x] Today checklist: tri-state controls, expand/collapse children, optimistic updates with rollup reflected
- [x] History — **Calendar view:** month grid with per-day status; click a day → that day's occurrence detail
- [x] History — **By-activity view:** pick an activity → its timeline/streak of states over a date range
- [x] "Connect Telegram" flow (deep-link button + connected state)
